...
#
[ "$(uname)" == "Darwin" ] && {
    echo "Detected Mac OS X; will use system-provided python and numpy."
    ...
    mkdir -p $EUPS_PATH/DarwinX86/numpy/system/ups
    $httpget $EUPS_PKGROOT/external/numpy/1.6.1/numpy.cfg > $EUPS_PATH/DarwinX86/numpy/system/ups/numpy.cfg
    eups declare python system -r $EUPS_PATH/DarwinX86/python/system -m none
    eups declare numpy system -r $EUPS_PATH/DarwinX86/numpy/system -m none
    eups declare tcltk system -r none -m none
