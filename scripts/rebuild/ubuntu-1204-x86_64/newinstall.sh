...
#
[[ "$(uname)" == "Linux" && -f /etc/redhat-release && $(sed -r 's/^.[^0-9.]*([0-9]*).*$/\1/' /etc/redhat-release) == 6 ]] && {
    echo "Detected a RHEL6 compatible system."
    echo "Using system tcl/tk"
    eups declare tcltk 8.5 -m none -r none
    cat > $EUPS_PATH/site/manifest.remap <<-EOF
        tcltk   8.5
EOF
}

# Ubuntu-specific setup: use as many system tools as possible
#
[[ -f /etc/lsb-release ]] && source /etc/lsb-release
[[ $DISTRIB_ID == "Ubuntu" ]] && {
    echo "Detected a Ubuntu compatible system."
    [[ $DISTRIB_RELEASE == "12.04" ]] && {
        echo "Using system tcl/tk, python, numpy, doxygen, and matplotlib."
        mkdir -p $EUPS_PATH/Linux64/python/system/ups
        $httpget $EUPS_PKGROOT/external/python/2.7.2/python.cfg > $EUPS_PATH/Linux64/python/system/ups/python.cfg
        mkdir -p $EUPS_PATH/Linux64/numpy/system/ups
        $httpget $EUPS_PKGROOT/external/numpy/1.6.1/numpy.cfg > $EUPS_PATH/Linux64/numpy/system/ups/numpy.cfg
        mkdir -p $EUPS_PATH/Linux64/gsl/system/ups
        $httpget $EUPS_PKGROOT/external/gsl/1.15/gsl.cfg > $EUPS_PATH/Linux64/gsl/system/ups/gsl.cfg
        eups declare python system -r $EUPS_PATH/Linux64/python/system -m none --force
        eups declare numpy system -r $EUPS_PATH/Linux64/numpy/system -m none --force
        eups declare gsl system -r $EUPS_PATH/Linux64/gsl/system -m none --force
        eups declare tcltk system -r none -m none --force
        eups declare matplotlib system -r none -m none --force
        eups declare doxygen system -r none -m none --force
        cat > $EUPS_PATH/site/manifest.remap <<-EOF
            python      system
            numpy       system
            gsl         system
